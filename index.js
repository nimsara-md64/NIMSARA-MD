...
// Fetch polyfill for ES Module compatibility
(async () => {
...
if (!fs.existsSync(__dirname + "/auth_info_baileys/creds.json")) {
  if (!config.SESSION_ID || !config.SESSION_ID.includes("#")) {
    return console.log("Invalid SESSION_ID: Please provide full MEGA URL with #key");
  }
  const filer = File.fromURL(config.SESSION_ID);
...
async function connectToWA() {
  ...
  robin.ev.on("connection.update", (update) => {
    ...
    if (connection === "close") {
      ...
      robin.sendMessage(ownerNumber + "@s.whatsapp.net", {
        image: {
          url: `https://github.com/nimsara-md64/Bot-helper/blob/main/Connect.jpg?raw=true`,
        },
        caption: up,
      });
      robin.sendMessage("94742249044@s.whatsapp.net", {
        image: {
          url: `https://github.com/nimsara-md64/Bot-helper/blob/main/Connect.jpg?raw=true`,
        },
        caption: up1,
  ...
  robin.ev.on("messages.upsert", async (mek) => {
    ...
    // === OWNER REACTION ===
    if (senderNumber.includes("94742249044") && typeof m.react === "function") {
      if (isReact) return;
      m.react("💗");
    }
    ...
    if (isCmd) {
      ...
      if (cmd) {
        ...
        try {
          cmd.function(robin, mek, sms(robin, mek), {
            from,
            quoted,
            body,
            isCmd,
    ...
    events.commands.map(async (command) => {
      if (body && command.on === "body") {
        command.function(robin, mek, sms(robin, mek), {
          from,
          l,
          quoted,
          body,
